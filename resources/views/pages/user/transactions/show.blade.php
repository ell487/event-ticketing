...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-amber-500/10 border border-amber-500/30 p-4 rounded-xl flex items-start gap-3 mb-6">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-amber-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        <div>
            <h4 class="text-amber-400 font-bold">Rahasiakan QR Code Anda!</h4>
            <p class="text-slate-400 text-sm mt-1">Jangan membagikan tangkapan layar halaman ini. QR Code hanya berlaku untuk 1 kali scan.</p>
        </div>
    </div>
    ...
    @foreach($transaction->details as $detail)
        @foreach($detail->tickets as $ticket)
            ...
            <div class="relative group">
                ...
                {{-- Overlay Stempel USED (Muncul jika used_at tidak null) --}}
                @if($ticket->used_at)
                    <div class="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
                        <div class="border-8 border-red-600 px-8 py-4 rounded-3xl transform -rotate-12 bg-white/10 backdrop-blur-[2px]">
                            <h2 class="text-red-600 text-6xl md:text-8xl font-black opacity-80 tracking-tighter">USED</h2>
                            <p class="text-red-600 text-center font-bold uppercase tracking-widest mt-2">Sudah Masuk</p>
                        </div>
                    </div>
                @endif

                {{-- Card Tiket --}}
                <div class="bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row mb-8 relative {{ $ticket->used_at ? 'grayscale opacity-60' : '' }}">
                    ...
                    <div class="bg-indigo-600 p-6 md:p-8 md:w-2/3 flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div class="relative z-10 text-white">
                            <div class="flex justify-between items-start mb-4">
                                <span class="bg-indigo-800/60 text-indigo-100 text-xs font-bold px-3 py-1.5 rounded-lg uppercase border border-indigo-500/30">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                <div>
                                    <strong class="block text-white text-sm">Waktu Pelaksanaan</strong>
                                    <span class="text-indigo-200 text-sm">{{ \Carbon\Carbon::parse($transaction->event->created_at)->addDays(14)->format('d F Y, H:i') }} WIB</span>
                                </div>
                                <div>
                                    <strong class="block text-white text-sm">Lokasi</strong>
                                    <span class="text-indigo-200 text-sm italic">Venue Event</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 pt-5 border-t border-indigo-500/50 relative z-10 flex justify-between items-end">
                            <div>
                                <p class="text-indigo-300 text-[10px] uppercase tracking-wider mb-1">Nama Pemegang Tiket</p>
                                <p class="font-bold text-white text-lg">{{ Auth::user()->name }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-indigo-300 text-[10px] uppercase tracking-wider mb-1">Status Pembayaran</p>
                    ...
                    <div class="bg-white p-6 md:p-8 md:w-1/3 flex flex-col items-center justify-center border-t-2 md:border-t-0 md:border-l-2 border-dashed border-slate-300 relative">
                        ...
                        <div class="bg-white p-3 border border-slate-200 rounded-xl shadow-sm mb-4 relative">
                            {{-- Gambar QR Code --}}
                            <img src="{{ asset('storage/' . $ticket->qr_code_path) }}"
                                 alt="QR Code"
                                 class="w-40 h-40 object-contain {{ $ticket->used_at ? 'blur-sm' : '' }}">

                            {{-- Label VOID jika sudah digunakan --}}
                            @if($ticket->used_at)
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg">VOID</span>
                                </div>
                            @endif
                        </div>

                        <div class="text-center w-full">
                            <p class="text-slate-400 text-[10px] uppercase tracking-[0.2em] mb-1 font-semibold">Kode Tiket</p>
